...
def _process_raw_dataset(filename: str, dirname: Path):
    ...
    # Verify the zip file is valid before processing
    try:
        with zipfile.ZipFile(filepath, "r") as zf:
            test_result = zf.testzip()
            if test_result is not None:
                raise zipfile.BadZipFile(f"Bad CRC in zip file: {test_result}")
    except (zipfile.BadZipFile, FileNotFoundError, OSError) as e:
        print(f"ERROR: Corrupted or invalid zip file detected: {e}")
        if filepath.exists():
            print(f"Deleting corrupted file: {filepath}")
            filepath.unlink()
        raise RuntimeError(
            f"The dataset file '{filepath}' is corrupted. It has been deleted. "
            "Please run prepare_data() again to re-download a fresh copy."
        ) from e
