...
def _download_raw_dataset(metadata: Dict, dl_dirname: Path) -> Path:
    ...
    while retry_count < max_retries:
        ...
        if filename.exists():
            ...
            print("Computing SHA-256 to verify file integrity...")
            sha256 = util.compute_sha256(filename)
            if sha256 == metadata["sha256"]:
                print("File integrity verified!")
                
                # Additional check: verify it's a valid zip file if it's a .zip
        ...
        # File doesn't exist or was deleted, download it
        print(f"Downloading raw dataset from {metadata['url']} to {filename}...")
        util.download_url(metadata["url"], filename)
        print("Computing SHA-256...")
        sha256 = util.compute_sha256(filename)
        if sha256 != metadata["sha256"]:
            print(f"Downloaded file SHA-256 does not match!")
            print(f"Expected: {metadata['sha256']}")
            print(f"Got: {sha256}")
    ...
    raise ValueError(f"Failed to download/verify dataset after {max_retries} attempts. Please try again.")
