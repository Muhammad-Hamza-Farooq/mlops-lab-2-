...
def _get_num_avail_cpus() -> int:
    """Return the number of CPUs available to this process, cross-platform."""
    try:
        # Linux: respects cpuset/affinity restrictions.
        sched_getaffinity = getattr(os, "sched_getaffinity", None)
        if sched_getaffinity is not None:
            return max(1, len(sched_getaffinity(0)))
    except Exception:
        pass
    # Windows/macOS: best-effort fallback.
    return max(1, os.cpu_count() or 1)


BATCH_SIZE = 128
NUM_AVAIL_CPUS = _get_num_avail_cpus()
