...
def _embed_crop_iam_line(crop: Image.Image, *, augment: bool) -> Image.Image:
...
class IAMLineStem(ImageStem):
    ...
    def __init__(self, augment=False, color_jitter_kwargs=None, random_affine_kwargs=None):
        ...
        # NOTE: Must be picklable for Windows `spawn` when DataLoader has num_workers>0.
        embed_fn = functools.partial(_embed_crop_iam_line, augment=augment)
        pil_transforms_list = [transforms.Lambda(embed_fn)]
        if augment:
            pil_transforms_list += [
                transforms.ColorJitter(**color_jitter_kwargs),
                transforms.RandomAffine(**random_affine_kwargs),
